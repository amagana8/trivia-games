...
func (s *Service) GetGridGamesByAuthorId(ctx context.Context, authorId string) (*[]model.GridGame, error) {
	gridGames, err := s.Repository.FindByAuthorId(ctx, authorId)
	if err != nil {
		fmt.Println("failed to get gridGames by authorId:", err)
		return nil, err
	}
