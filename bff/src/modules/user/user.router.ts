...
export const userRouter = router({
  changePassword: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const newTokens = await userService.changePassword({ newPassword: input.password, userId: ctx.userId });
      sendAuthTokens(ctx.res, newTokens);

      return;
    }),
  getMe: protectedProcedure.query(({ ctx }) => userService.getMe({ userId: ctx.userId })),
  signIn: publicProcedure.input(signUpRequestValidator.omit({ email: true })).mutation(async ({ input, ctx }) => {
  ...
  signUp: publicProcedure.input(signUpRequestValidator).mutation(async ({ input, ctx }) => {
